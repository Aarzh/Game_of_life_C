...
void allocateImage(pgm_t * board)
{
    int i;
    board->image.pixels = (pixel_t **)malloc(board->image.height*sizeof(pixel_t *));
	if(board->image.pixels==NULL)
	{
		printf("Problem in memory allocation of image->pixels1!\n");
		printf("Program exited.\n");
		return;
	}
    board->image.pixels[0] = calloc(board->image.height * board->image.width, sizeof(pixel_t *));
    for(i = 0; i < board->image.height; i++)
	{
        board->image.pixels[i] = board->image.pixels[0] + board->image.width * i;
		//board->image.pixels[i] = (pixel_t *) calloc(board->image.width, sizeof(pixel_t));
		if(board->image.pixels[i]==NULL)
		{   
			printf("Problem in memory allocation of board->image.pixels!\n");
			printf("Program exited.\n");
			return;
		}
	}
...
void freeImage(pgm_t * board)
{
    free(board->image.pixels[0]);
    free(board->image.pixels);
    //int i;
    // Free the memory used
    /*for (i=0; i<board->image.height; i++)
    {
        free(board->image.pixels[i]);
    }
    free(board->image.pixels);
    // Set the sizes to zero
    board->image.pixels = NULL;
    board->image.height = 0;
    board->image.width = 0;*/
}

// Copy an image to another structure
void copyPGM(pgm_t * source, pgm_t * destination)
{
